"""Single-attempt adapter around the existing safe skill clients. No credential reads.
...
PRIVATE = ROOT / "output/imagegen/scene-production/.private/jobs"
...
def inspect(directory, error_code=None):
    ...
    for path, expected_hash, expected_bytes in paths:
        try:
            ...
            continue
    if error_code is None and not recs:
        # The paid marker is fsynced before the provider client can start. Inspect
        # a prepared-only directory after its worker has exited; retain any other
        # artifact as uncertainty instead of inferring that nothing was submitted.
        try:
            entries = list(directory.iterdir())
            allowed = {"prompt.txt", "request.json", "result.json", "no-post-resolution.json"}
            spec = json.loads((directory / "request.json").read_text(encoding="utf-8"))
            prompt = (directory / "prompt.txt").read_bytes()
            if (entries and all(p.is_file() and not p.is_symlink() and p.name in allowed for p in entries)
                    and hashlib.sha256(prompt).hexdigest() == spec.get("promptHash")
                    and re.fullmatch(r"[a-f0-9]{64}", str(spec.get("sourceHash", "")))):
