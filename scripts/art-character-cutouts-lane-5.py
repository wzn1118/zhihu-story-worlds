...
def process(entry):
    ...
    history = WORK / 'five-threads/lane-5-history'
    history.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
    review_path = WORK / 'reviews' / (entry['jobId'] + '.json')
    for prior, suffix in [(metadata, 'result'), (review_path, 'review')]:
        if prior.exists():
            (history / (name + '-' + stamp + '-' + suffix + '.json')).write_bytes(prior.read_bytes())
    ...
    saved = np.asarray(Image.open(target).convert('RGBA'))
    if saved.shape[:2] != (rgb.height, rgb.width) or not np.array_equal(saved[:,:,:3],np.asarray(rgb)):
        raise ValueError('RGB_OR_DIMENSION_MISMATCH')
...
def main():
    ...
    parser.add_argument('--sheet-start',type=int)
    ...
    rows=plan['entries']
    if args.sheet_start:
        start=args.sheet_start-1
        sheet=Image.new('RGB',(1440,1160),'#bbb'); draw=ImageDraw.Draw(sheet)
        for j,e in enumerate(rows[start:start+4]):
            key=e['jobId']+'-'+e['asset']['sha256'][:12]
            im=Image.open(WORK/'previews'/(key+'.jpg')); im.thumbnail((720,550))
            x=j%2*720; y=j//2*580; sheet.paste(im,(x,y+28))
            draw.text((x+8,y+5),str(start+j+1)+' '+e['worldId']+' '+e['nodeId'],fill='black')
        sheet.save(WORK/'five-threads'/('lane-5-full-%02d.jpg'%args.sheet_start),quality=94)
        return
    save_json(WORK/'five-threads/started-5.json',dict(lane=5,pid=os.getpid(),at=datetime.now(timezone.utc).isoformat(),planFile=args.plan_file,workers=args.workers,status='processing'))
