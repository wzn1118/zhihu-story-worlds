...
async function main() {
  ...
  async function checkpoint<T>(label: string, schema: Schema, prompt: string) {
    ...
    const result = await runCreative<T>(join(directory, 'creative'), label, schema, prompt, async pid => { childPid = pid; await heartbeat(); });
  ...
  try {
    ...
    const outline = await checkpoint<StoryOutline>('outline', outlineSchema, outlinePrompt(source));
    for (const fact of outline.facts) if (!source.text.includes(fact.quote)) throw new Error('事实阶段引用与原文不一致；需要重新生成此版。');
    let draft: GeneratedDraft = { outline, routes: [] };
