...
import { ArrowRight, BookOpen, Bookmark, Check, ChevronDown, ChevronUp, Download, FilePlus2, LoaderCircle, Pause, Play, RefreshCw, Search, Upload } from 'lucide-react';
...
import { ZhihuFavorites } from './ZhihuFavorites';
...
import { WorkshopRelaySettings, type WorkshopRelayInput } from './WorkshopRelaySettings';
...
function readOptions(): WorkshopGenerationOptions {
  try { const value = JSON.parse(accountLocalStorage.getItem(optionsKey) ?? 'null'); if (value && ['fast', 'full'].includes(value.mode) && ['faithful', 'inspiration'].includes(value.adaptation) && ['none', 'image2', 'gpt6'].includes(value.images)) return value; } catch { /* Private browsing can disable storage. */ }
  return defaultGenerationOptions;
}
const imageEndpoint = (id: string) => `/api/workshop/images/${encodeURIComponent(id)}`;
function readInput(): ImportedSource { try { const input = JSON.parse(accountLocalStorage.getItem(draftKey) ?? 'null'); if (input && typeof input.title === 'string' && typeof input.author === 'string' && typeof input.text === 'string' && ['user-import', 'original-seed'].includes(input.scope)) return input; } catch { /* private browsing */ } return blank; }
...
export function ImportedSourceReader({ source }: { source: ImportedSource }) {
  ...
  return <article className="imported-reader" ref={reader}>
    ...
    <header>{source.scope === 'zhihu-excerpt' ? <ZhihuBadge label={source.origin?.contentScope === 'favorite-summary' ? '收藏摘要' : '原作节选'} /> : <p className="eyebrow">{sourceScopeLabel(source.scope, source.origin)}</p>}<h2>{source.title}</h2><AuthorIdentity name={source.author} avatar={source.origin?.authorAvatar} />
      <p>{source.origin?.contentScope === 'favorite-summary' ? '这是你从知乎收藏中选取的摘要，由知乎收藏接口提供，并非完整原作。' : source.origin?.contentScope === 'webpage-selection' ? '这是你在知乎网页选取的可见正文，按浏览器传来的文字逐字保存，可能只是原作的一部分。' : source.origin?.contentScope === 'search-excerpt' ? '这是知乎官方搜索返回的节选，逐字保存，并非完整原作。' : source.scope === 'zhihu-excerpt' ? '这份知乎原作节选在创建改编项目时逐字保存，并非完整作品。' : source.scope === 'original-seed' ? '这是工作台的原创短篇示例，不是知乎原文。' : '以下是你导入的原文，未作改写，可能只是作品节选。'}游戏中新增的情节与结局属于 AI 改编。</p><button className="text-button" onClick={download}><Download size={13} />下载{source.origin?.contentScope === 'favorite-summary' ? '收藏摘要' : '原文'} · UTF-8</button>
      {source.origin && <SourceLinks source={source} />}
    </header><pre data-testid="imported-source-text">{segments.map((segment, index) => segment.matchIndex === undefined ? segment.text : <mark key={index} data-import-match={segment.matchIndex} data-active={selected === segment.matchIndex}>{segment.text}</mark>)}</pre><p className="imported-reader-end">{source.origin?.contentScope === 'favorite-summary' ? '收藏摘要到这里结束。' : '原文到这里结束。'}</p>
  </article>;
}

export function StoryWorkshop({ onPlay, onRead, stories, onReadZhihu, requestedSource, requestedProjectId, onBrowseZhihu, tourStep, allowServiceConfiguration = true }: {
  ...
  const [canEditCreativeConfig, setCanEditCreativeConfig] = useState(false);
  ...
  const [sourceTab, setSourceTab] = useState<'zhihu' | 'paste' | 'discover' | 'favorites'>(() => location.hash.startsWith('#zhihu-page=') ? 'discover' : 'paste');
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const data = await fetchJson<{ projects: WorkshopProject[]; capabilities?: { creativeConfigEditable?: boolean; quickImages?: boolean; revise?: boolean; zhihuImport?: boolean; environmentRelay?: typeof environmentRelay } }>('/api/workshop/projects');
      if (sequence !== refreshSequence.current) return;
      setQuickImages(data.capabilities?.quickImages === true);
      setCanEditCreativeConfig(data.capabilities?.creativeConfigEditable !== false);
      ...
      setLoading(false);
      if (!selected) return;
      if (!data.projects.some(p => p.id === selected)) { setSelected(null); return; }
      // Publish the successful list before fetching detail: one stale selection
      // must not prevent other projects from showing their latest progress.
      try {
        const detail = await fetchJson<WorkshopProject>(`/api/workshop/projects/${encodeURIComponent(selected)}`);
        if (sequence !== refreshSequence.current) return;
        setProjects(data.projects.map(p => p.id === selected ? detail : p));
      } catch (e) {
        if (sequence !== refreshSequence.current) return;
        if (e instanceof ApiError && e.status === 404) {
          setProjects(data.projects.filter(p => p.id !== selected)); setSelected(null);
  ...
  useEffect(() => { try { accountLocalStorage.setItem(draftKey, JSON.stringify(input)); } catch { /* server preserves imports */ } }, [input]);
  useEffect(() => { try { accountLocalStorage.setItem(optionsKey, JSON.stringify(generationOptions)); } catch { /* Project keeps the accepted options. */ } }, [generationOptions]);
  ...
  const saveRelay = async (config: WorkshopRelayInput): Promise<boolean> => {
    let saved = false;
    await act(async () => {
      const response = await post<{ creativeTransport: 'relay' | 'local-cli'; relay: typeof relay }>('/api/workshop/creative-config', config);
      setCreativeTransportName(response.creativeTransport); setRelay(response.relay); setRelayMessage('中转站配置已保存，新的生成会使用它。'); saved = true;
    });
    return saved;
  };
  ...
  return <section className={`story-workshop ${dragging ? 'workshop-dragging' : ''}`} aria-label="新故事生成工作台" onDragOver={event => { if (event.dataTransfer.types.some(type => ['Files', 'text/plain', 'text/html'].includes(type))) { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; setDragging(true); } }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false); }} onDrop={dropSource}>
    ...
    <div className="workshop-layout"><div className="workshop-source-column">
      ...
      <div className="source-mode-tabs" role="tablist" aria-label="原文来源">{([{ id: 'discover', label: '发现新故事' }, { id: 'zhihu', label: '从知乎选篇' }, { id: 'favorites', label: '我的知乎收藏' }, { id: 'paste', label: '粘贴或上传' }] as const).map((tab, index, tabs) => <button key={tab.id} type="button" id={`source-tab-${tab.id}`} role="tab" aria-selected={visibleSourceTab === tab.id} aria-controls={`source-panel-${tab.id}`} tabIndex={visibleSourceTab === tab.id ? 0 : -1} onClick={() => setSourceTab(tab.id)} onKeyDown={event => { if (['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) { event.preventDefault(); const next = tabs[event.key === 'Home' ? 0 : event.key === 'End' ? tabs.length - 1 : (index + (event.key === 'ArrowRight' ? 1 : -1) + tabs.length) % tabs.length].id; setSourceTab(next); document.getElementById(`source-tab-${next}`)?.focus(); } }}>{tab.id === 'discover' ? <Search size={16} /> : tab.id === 'zhihu' ? <BookOpen size={16} /> : tab.id === 'favorites' ? <Bookmark size={16} /> : <Upload size={16} />}{tab.label}</button>)}</div>
      {visibleSourceTab === 'favorites' ? <div id="source-panel-favorites" role="tabpanel" aria-labelledby="source-tab-favorites"><ZhihuFavorites generationOptions={generationOptions} projects={projects} onProject={p => { replace(p); setSelected(p.id); requestAnimationFrame(() => document.querySelector('.workshop-detail')?.scrollIntoView({ block: 'center', behavior: 'smooth' })); }} onCompleteSource={source => { setInput(source); setSourceUrl(source.referenceUrl ?? ''); setSourceTab('paste'); setImportNotice('已带入收藏摘要和来源链接，请补充至少 80 字正文后生成。'); }} /></div> : visibleSourceTab === 'discover' ? <div id="source-panel-discover" role="tabpanel" aria-labelledby="source-tab-discover"><ZhihuDiscovery generationOptions={generationOptions} projects={projects} onProject={p => { replace(p); setSelected(p.id); requestAnimationFrame(() => document.querySelector('.workshop-detail')?.scrollIntoView({ block: 'center', behavior: 'smooth' })); }} /></div> : visibleSourceTab === 'zhihu' ? <div id="source-panel-zhihu" role="tabpanel" aria-labelledby="source-tab-zhihu"><ZhihuSourcePicker stories={stories} selected={selectedSource} onSelect={setSelectedSource} onRead={onReadZhihu} onImport={story => void importZhihu(story.id)} busy={busy} available={canImportZhihu} /></div> : <form id="source-panel-paste" role="tabpanel" aria-labelledby="source-tab-paste" className="workshop-form" data-tour="workshop-source-input" onSubmit={event => { event.preventDefault(); void submit(true); }}>
      ...
      {allowServiceConfiguration && canEditCreativeConfig && <details className="relay-config" open={tourStep ? tourStep.id === 'relay' : undefined}>
        <summary data-tour={tourStep?.id === 'relay' ? undefined : 'workshop-relay'}>配置创作中转站</summary>
        <WorkshopRelaySettings busy={busy} relay={relay} environmentRelay={environmentRelay} message={relayMessage} onSave={saveRelay} onClear={clearRelay} onUseEnvironment={useEnvironmentRelay} />
      </details>}
      ...
      {project && <article className="workshop-detail" aria-live="polite"><div data-tour="workshop-progress"><header><p className="eyebrow">{project.id} / {project.revision ? `当前稿 r${project.revision}` : '尚未建稿'} / {project.publishedVersion ? `已发布 ${project.publishedVersion}` : '尚未发布'}</p><h2>{project.title}</h2><p>{statusNames[project.status]} · {stageNames[project.stage]}{project.status === 'running' && ' · 作业在后台运行'}</p></header>
        ...
        {ownBatch && <section className="workshop-art-panel" data-project-id={ownBatch.storyId} data-world-version={ownBatch.worldVersion}><h3>{ownBatch.worldTitle} · {ownBatch.worldVersion} 插图清单</h3><p>总计 {ownBatch.progress.current} · 已生成 {ownBatch.progress.generated} · 原生4K {ownBatch.progress.native4k} · 已审核覆盖 {ownBatch.progress.covered} · 未确认 {ownBatch.progress.unknownOutcome} · 失败 {ownBatch.progress.failed}</p>{ownBatch.circuitBreaker && <p>服务已暂停：{ownBatch.circuitBreaker.code}</p>}<div className="workshop-actions"><button className="secondary-button" disabled={busy || ownBatch.state === 'running' || !!ownBatch.circuitBreaker || !ownBatch.progress.queued} onClick={() => void artRun('run')}><Play />生成场景插图{ownBatch.progress.queued > 0 ? ` · ${Math.min(40, ownBatch.progress.queued)} 张` : ''}</button><button className="text-button" disabled={busy || ownBatch.state !== 'running'} onClick={() => void artRun('pause')}><Pause />暂停排队</button><button className="text-button" disabled={busy || ownBatch.state === 'running' || (!ownBatch.progress.recoverable && !ownBatch.progress.unknownOutcome)} onClick={() => void artRun('run', true)}>恢复已返回图片</button></div>
          <details><summary>逐场景状态与人工审核</summary><label>审核人<input value={reviewer} onChange={e => setReviewer(e.target.value)} /></label><label>具体观察<textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="核对人物一致性、场景内容、硬边面部阴影、背景与原生尺寸" /></label><div className="art-review-grid">{ownBatch.jobs.filter(j => !j.stale).map(j => <article key={j.id}><h4>{j.sceneTitle}</h4><p>{j.state} / {j.review?.decision ?? '未审核'}{j.errorCode ? ` / ${j.errorCode}` : ''}</p>{j.asset && /^\/generated-art\/workshop\/wscene_[a-f0-9]+\.png$/.test(j.asset.url) && <><a href={j.asset.url} rel="noreferrer"><Artwork src={j.asset.url} alt={`${j.sceneTitle} · ${j.review?.decision === 'approved' ? '审核通过' : j.review?.decision === 'rejected' ? '已退回' : '待审核'}`} /></a><p>{j.asset.width} × {j.asset.height} · {j.asset.native4k ? '原生4K候选' : '实际返回尺寸'}</p><button className="text-button" disabled={busy || !reviewer.trim() || !notes.trim() || (!j.asset.native4k && ownBatch.qualityPolicy !== 'style-first')} onClick={() => void review(j.id, 'approved')}><Check />通过</button><button className="text-button" disabled={busy || !reviewer.trim() || !notes.trim()} onClick={() => void review(j.id, 'rejected')}>退回</button></>}</article>)}</div></details>
        </section>}
      </article>}
    </div></div>
  </section>;
