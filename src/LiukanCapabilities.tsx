...
async function api<T>(path: string, input?: unknown): Promise<T> {
  const response = await fetch(`/api/liukan/${path}`, { ...(input === undefined ? {} : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }) });
...
export function LiukanCapabilities({ onClose, onOpenSource, onReadingDesk, onActivityDesk }: LiukanCapabilitiesProps) {
  ...
  return <section className="liukan-capabilities" aria-label="看山的能力" data-guide="liukan-capabilities">
    ...
    <nav className="liukan-cap-tabs" aria-label="看山能力分类">{([['chat', '聊一聊'], ['tools', '找资料'], ['settings', '对话设置']] as const).map(([id, label]) => <button key={id} type="button" aria-pressed={tab === id} onClick={() => changeTab(id)}>{label}</button>)}</nav>
    {tab === 'chat' && <div className="liukan-cap-chat">
      ...
      <small className="liukan-cap-provider">{config?.transport === 'relay' ? `当前中转 · ${config.model}` : `知乎直答 · ${config?.model ?? '正在读取配置'}`}</small>
    </div>}
    {tab === 'tools' && <div className="liukan-cap-tools">
      <label>让看山帮你<select disabled={busy} value={ability} onChange={event => { setAbility(event.target.value as LiukanAbilityId); setPrivateAccess(false); setResult(null); setError(''); }}>{LIUKAN_ABILITIES.map(row => <option key={row.id} value={row.id}>{row.title}</option>)}</select></label>
    ...
    {tab === 'settings' && <div className="liukan-cap-settings" data-guide="liukan-relay-settings">
