...
```text
https://zhihu.hegelsalon.com/auth/callback
```
...
打开 <http://127.0.0.1:4179/> 预览页面。如果回调地址显示“部署后配置”，请在部署完成后运行 `zhihu-hackathon` Skill 提供的 `configure_callback.mjs`。只有部署后的公网页面可以测试授权，最终确认必须由用户本人完成。
